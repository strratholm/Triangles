...
Line::Line(const Plane &pl1, const Plane &pl2) {
    ...
    double coordpl[2][4] = {{pl1.normal.x, pl1.normal.y, pl1.normal.z, pl1.d},
                            {pl2.normal.x, pl2.normal.y, pl2.normal.z, pl2.d}};

    double coords[2];

    int jind1 = (!isWithinError(coordpl[0][ind1], 0, eps, 0)) ? 0 : 1;
    int jind2 = (jind1 + 1) % 2;

    coords[1] = (coordpl[jind1][3] * coordpl[jind2][ind1] / coordpl[jind1][ind1] - coordpl[jind2][3]) /
                (coordpl[jind2][ind2] - coordpl[jind1][ind2]*coordpl[jind2][ind1]/coordpl[jind1][ind1]);

    coords[0] = (-coordpl[jind1][3] - coordpl[jind1][ind2] * coords[1]) / coordpl[jind1][ind1];
...
Vector3 Line::getIntersection(const Line &line1, const Line &line2) {
    ...
    Vector3 from_base2_to_inter = Vector3();
